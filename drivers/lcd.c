...
void lcd_display_dec(uint16_t value)
{
  if (value > 9999) {
    ...
    lcd_set(division_asm(value,1000), 1);
    lcd_set(division_asm((value - division(value,1000)*1000),100), 2);
    lcd_set(division_asm(value - division(value,100)*100,10), 3);
    lcd_set(value - division(value,10)*10, 4);
  }
}
